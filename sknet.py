...
class SKConv(nn.Module):
    ...
    # TODO: Explain SKConv Block Properly
    def __init__(self, features, M=2, G=32, r=16, stride=1, L=32):
        ...
        for i in range(M):
            ...
            #    self.convs.append(nn.Sequential(
            #        nn.Conv2d(features, features, kernel_size=3, stride=stride,
            #                  padding=1, dilation=1, groups=G, bias=False),
            #        nn.BatchNorm2d(features),
            #        nn.ReLU(inplace=True)
            #    ))
            #if i == 1:
            #    self.convs.append(nn.Sequential(
            #        nn.Conv2d(features, features, kernel_size=5, stride=stride,
            #                  padding=2, dilation=1, groups=G*2, bias=False), # padding =4, dilation=2, groups=G is another configuration
            #        nn.BatchNorm2d(features),
            #        nn.ReLU(inplace=True)
            #    ))
...
class SKNet(nn.Module):
    ...
    # TODO: Explain SKNet Architecture here
    def __init__(self, class_num, nums_block_list=[3, 4, 6, 3], strides_list=[1, 2, 2, 2]):
        ...
        self.stage_1 = self._make_layer(
            64, 128, 256, nums_block=nums_block_list[0], stride=strides_list[0])
        self.stage_2 = self._make_layer(
            256, 256, 512, nums_block=nums_block_list[1], stride=strides_list[1])
        self.stage_3 = self._make_layer(
            512, 512, 1024, nums_block=nums_block_list[2], stride=strides_list[2])
        self.stage_4 = self._make_layer(
            1024, 1024, 2048, nums_block=nums_block_list[3], stride=strides_list[3])
